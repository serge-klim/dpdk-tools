...
void shutdown() noexcept {
    BOOST_LOG_SEV(log::get(), boost::log::trivial::warning) << "shutting down...";
    dpdkx::stop_jobs();     
    unsigned int lcore_id;
    RTE_LCORE_FOREACH_WORKER(lcore_id) {
        if (rte_eal_wait_lcore(lcore_id) < 0)
            BOOST_LOG_SEV(log::get(), boost::log::trivial::error) << "rte_eal_wait_lcore("<<lcore_id<<") failed : " <<dpdkx::last_error().message() << "...";
    }   
}    
...
void run(boost::program_options::variables_map const& options) {
    ...
    auto svc_mempools = std::vector<dpdkx::scoped_mempool>{};
    svc_mempools.push_back(dpdkx::make_scoped_mempool(dpdkx::svc_mempool_name(socket_config->socket_id).c_str(), 64 - 1, 24, 0, RTE_PKTMBUF_HEADROOM * 2 /*RTE_MBUF_DEFAULT_BUF_SIZE*/, socket_config->socket_id));
    ...
    if(rte_eth_dev_set_ptypes(device.port_id(), RTE_PTYPE_UNKNOWN, NULL, 0) < 0)
        BOOST_LOG_SEV(log::get(), boost::log::trivial::error) << "unable to disable Ptype parsing: " << dpdkx::last_error().message() << "...";
    ...
    if(rte_eth_promiscuous_enable(device.port_id())!=0)
        BOOST_LOG_SEV(log::get(), boost::log::trivial::error) << "port " << device.port_id() <<" rte_eth_promiscuous_enable failed : " << dpdkx::last_error().message();
