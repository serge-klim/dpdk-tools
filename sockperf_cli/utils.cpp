...
void timestamp_packet(dpdkx::device& /*device*/, rte_mbuf* buffer, std::uint64_t ol_flags, std::size_t payload_size) {
...
void prepare_packet(dpdkx::device& device, sockperf::seqn_t seqn, sockperf::type type, rte_mbuf* buffer, std::uint64_t ol_flags, std::size_t payload_size) {
...
dpdkx::job_state latency_test_job::process() {
    ...
    auto port_id = rx_channel_.device().port_id();
    auto const use_dev_clock = (rx_channel_.device().rx_offloads() & RTE_ETH_RX_OFFLOAD_TIMESTAMP) == RTE_ETH_RX_OFFLOAD_TIMESTAMP;
//    auto buffer = std::unique_ptr<rte_mbuf, decltype(&rte_pktmbuf_free)>{ nullptr, &rte_pktmbuf_free };
    ...
    while (seqn < packets2send_ && !dpdkx::jobs_suspended()) {
        if (auto buffer = rte_pktmbuf_alloc(packet_pool_.get())) {
            prepare_packet(rx_channel_.device(), sockperf::x::warmup(seqn), sockperf::type::/*Warmup*/Ping, buffer, ol_flags_, payload_size_);
            auto dev_clock = use_dev_clock ? rx_channel_.device().read_clock() : dpdkx::ns_timer();
                    ...
                    if (auto msg = rx_channel_.last_warmup_message()) {
                        if (msg->seqn == sockperf::x::warmup(seqn)) {
                            BOOST_LOG_SEV(log::get(), boost::log::trivial::info) << " reply time = " 
                                                       << std::chrono::duration_cast<std::chrono::duration<double,std::micro>>(std::chrono::nanoseconds{ msg->received_ts - dev_clock }).count() << " us.";
                            break;                            
                        }
                    }
                    if (i-- == 0) {
...
{ 
    auto const enqueued = dpdkx::ns_timer(); // increses missed packets from 0-0.2% to 3% on small packets almost no impact on packets 512+ bytes
    ...
    ///opt/dpdk.prof/bin/dpdk-testpmd -a 0000:01:00.1 -l 6,8 -- --forward-mode=mac
    // ---------------------- Forward statistics for port 0  ----------------------
    // RX-packets: 1000000        RX-dropped: 0             RX-total: 1000000
    // TX-packets: 985687         TX-dropped: 14313         TX-total: 1000000
    // ----------------------------------------------------------------------------
        ...
        switch (auto type = sockperf::message_type(payload, buffer->data_len - msg_header_size)) {
            ...
            case sockperf::type::Pong: {
                ...
                if (unlikely(sockperf::x::is_warmup(seqn))) {
                    ...
                    received_msg_info->received_ts = timestamp ? device().timestamp_fix(*timestamp) : enqueued;
