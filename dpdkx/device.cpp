...
constexpr bool is_multicast_ipv4_addr(rte_be32_t ipv4_addr) noexcept {
   return (ipv4_addr /*(rte_be_to_cpu_32(ipv4_addr) >> 24)*/ & 0x000000FF) == 0xE0;
}
...
dpdkx::job_state dpdkx::device::tx_job::process() {
   ...
   if (likely(burst_size_ == n_ ||
      ...
      auto sent = rte_eth_tx_burst(device_.port_id(), queue_id_, buffers_.data(), n_);
      if (likely(sent != 0)) {
         if (unlikely(n_ != sent)) {
...
dpdkx::job_state dpdkx::device::rx_job::process() {
   ...
   auto find_rx_channel = [this](ip4_endpoint const& ep) -> rx_channel* {
   ...
   try_reload_sinks();
   auto res = job_state::idling;
   if (const auto n = rte_eth_rx_burst(device_.port_id(), queue_id_, buffers_.data(), burst_size_)) {
#pragma clang loop vectorize(enable) interleave(enable)
      ...
      auto channel = static_cast<rx_channel*>(nullptr);
      auto ready = 0;
      auto post_process = n;
         ...
         switch (eth_proto /*update_l2size*/ (buffers_[i])) {
            ...
            case RTE_BE16(RTE_ETHER_TYPE_IPV4): {
               ...
               if (ipv4hdr->next_proto_id == IPPROTO_UDP) {
                  ...
                  if (auto ch = find_rx_channel(ip4_endpoint{std::bit_cast<rte_be32_t>(ipv4hdr->dst_addr), std::bit_cast<decltype(udphdr->dst_port)>(udphdr->dst_port)})) {
                     buffers_[i]->l3_len = l3size /*sizeof(*ip_hdr)*/;
                     buffers_[i]->l4_len = sizeof(struct rte_udp_hdr);
                     if (ch != channel) {
                        if (ready != 0) {
                           if (auto recycle = channel->enqueue(queue_id_, buffers_.data() + i - ready, ready))
                              rte_pktmbuf_free_bulk(buffers_.data() + i - ready, recycle);
                        }
                        channel = ch;
                        ready = 0;
                     }
            ...
            default:
               buffers_[post_process++] = buffers_[i];
         }
         if (ready != 0) {
            channel->enqueue(queue_id_, buffers_.data() + i - ready, ready);
            channel = nullptr;
            ready = 0;
         }
      }
      if (ready != 0)
         channel->enqueue(queue_id_, buffers_.data() + n - ready, ready);

      if (n != post_process) {
         auto begin = buffers_.data() + n;
         if (auto recycle = device_.process_rx_packets(queue_id_, begin, post_process - n))
            rte_pktmbuf_free_bulk(begin, recycle);
         if (n == post_process - n && post_process - n != burst_size_)
...
dpdkx::device::device(config::device config)
    ...
    , tx_queues_{ static_cast<std::uint16_t>(config_.tx_rings > config_.info.default_txportconf.nb_queues ? 0 : config_.info.default_txportconf.nb_queues - config_.tx_rings) }
{   
   std::ranges::fill(per_core_queue_, no_txq);
   ...
   if (config_.rx_flow_queues != 0) {
      if (config_.rx_flow_queues > config_.info.default_rxportconf.nb_queues)
         config_.rx_flow_queues = config_.info.default_rxportconf.nb_queues;
       fqueues_ = std::ranges::views::iota(static_cast<queue_id_t>(config_.info.default_rxportconf.nb_queues - config_.rx_flow_queues), static_cast<queue_id_t>(config_.info.default_rxportconf.nb_queues)) 
                    | std::ranges::views::transform([this](auto const& queue_ix) { return flow::rx_queue{config_, queue_ix}; })
                    | std::ranges::to<decltype(fqueues_)>();

   }

   assert(config_.info.default_rxportconf.nb_queues >= config_.rx_flow_queues);
   auto n_rss_queues = static_cast<queue_id_t>(config_.info.default_rxportconf.nb_queues - config_.rx_flow_queues);
    rx_jobs_ = utils::workarounds::to<decltype(rx_jobs_)>(std::ranges::views::iota(queue_id_t{0}, queue_id_t{n_rss_queues /*config_.info.default_rxportconf.nb_queues*/})
   ...
   if (config_.info.default_txportconf.nb_queues - tx_queues_ != 0) {
      logging::logger<dpdkx::device>().info("port {} : {} setting up {}/{} (tx queues/rings)", config.port_id, config.info.driver_name, config_.info.default_txportconf.nb_queues, config_.info.default_txportconf.nb_queues - tx_queues_);
        tx_jobs_ = utils::workarounds::to<decltype(tx_jobs_)>(std::ranges::views::iota(queue_id_t{ tx_queues_ }, queue_id_t{ config_.info.default_txportconf.nb_queues })
   ...
   if (n_rss_queues /*config_.info.default_rxportconf.nb_queues*/ > 1) {
   ...
   if (rte_eth_dev_configure(port_id(), config_.info.default_rxportconf.nb_queues, config_.info.default_txportconf.nb_queues, &port_conf) != 0)
      throw std::system_error{dpdkx::last_error(), "rte_eth_dev_configure failed"};
   ...
   const auto nb_mbuf_per_pool = static_cast<unsigned int>(RX_DESC_MAX + (config_.info.default_rxportconf.nb_queues /*nb_lcores*/ * mb_mempool_cache) + TX_DESC_MAX + max_pkt_burst);
   ...
   if (config_.svc_tx_n > tx_jobs_.size() /*config_.info.default_txportconf.nb_queues*/)
      config_.svc_tx_n = tx_jobs_.size() /*config_.info.default_txportconf.nb_queues*/;
}

dpdkx::device::~device() {
   if (rx_mbuf_pool_) { // if it's false object was moved, probably should, to implement move constructor/operator but...too lazy:(
...
std::vector<dpdkx::job*> dpdkx::device::jobs() {
   ...
   std::transform(begin(fqueues_), end(fqueues_), std::back_inserter(res), [](auto& job) {
      return &job;
   });
...
int dpdkx::device::link_status(rte_eth_link& status, std::chrono::milliseconds timeout) const noexcept {
   auto [effective_timeout, retries] = retry(timeout);
   assert(retries != 0);
   do {
      auto res = rte_eth_link_get_nowait(port_id(), &status);
      if (res == 0 && status.link_status != RTE_ETH_LINK_DOWN)
         return res;
      rte_delay_ms(static_cast<unsigned int>(effective_timeout.count()));
   } while (--retries);
   return 0 /*ETIMEDOUT*/;
}
...
int dpdkx::device::start() /*noexcept*/ {
   ...
   auto res = rte_eth_dev_start(port_id());
   if (res == 0)
      std::ranges::for_each(fqueues_, &flow::rx_queue::start);
   return res;
}
...
dpdkx::queue_id_t dpdkx::device::tx_queue(core_t core_id) noexcept {
   auto res = per_core_queue_[core_id];
   if (res == no_txq && tx_queues_ != 0) {
      res = --tx_queues_;
      logging::logger<dpdkx::device>().info("assigning tx queue: {} to core :", res, core_id);
   }
   return res;
}

dpdkx::tx_ring& dpdkx::device::service_tx(queue_id_t hint /*= 0*/) noexcept {
   assert(config_.svc_tx_n <= tx_jobs_.size());
   auto ix = hint % config_.svc_tx_n + tx_jobs_.size() - config_.svc_tx_n;
   return tx_jobs_[ix].tx_ring();
}

dpdkx::tx_ring& dpdkx::device::tx_ring(queue_id_t hint /*= 0*/) noexcept {
...
std::error_code dpdkx::device::attach_rx(ip4_endpoint ep, std::shared_ptr<rx_channel> ch, attach_mode mode /*= attach_mode::any*/) {
   if (mode != attach_mode::rss) {
      auto i = std::ranges::find_if(fqueues_, [ch](auto& queue) {
         return ch == queue.channel();
      });
      if (i != end(fqueues_)) {
         if (auto error = i->route_to(ep))
            return error;
      ...
      if (mode == attach_mode::flow)
         return make_error_code(dpdkx::errc::no_avalible_queue);
   }
   auto updated = false;
   do {
      auto active = sinks();
      ...
      updated = workaround::compare_exchange_strong(sinks_, active, std::move(sinks));
   } while (!updated);
   return RTE_IS_IPV4_MCAST(rte_be_to_cpu_32(ep.first))
              ? join_mcast(ep.first)
              : std::error_code{};
...
std::error_code dpdkx::device::detach_rx(ip4_endpoint ep, std::shared_ptr<rx_channel> ch) {
   auto attempt = 0;
   auto updated = false;
   do {
      auto active = sinks();
